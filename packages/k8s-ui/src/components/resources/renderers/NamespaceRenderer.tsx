...
export function NamespaceRenderer({ data, costData }: NamespaceRendererProps) {
  ...
  const costTotal = costData ? costData.cpuCost + costData.memoryCost + storageCost : 0
      ...
      {costData && (
        <Section title="Cost" icon={DollarSign} defaultExpanded>
          ...
          <div className="mt-3 pt-3 border-t border-theme-border space-y-2">
            ...
            {storageCost > 0 && (
              <div className="flex items-center justify-between text-xs">
                <span className="text-theme-text-tertiary">Storage</span>
                <span className="text-theme-text-secondary tabular-nums">{formatCost(toDailyCost(storageCost))}/day</span>
              </div>
            )}
            ...
            {costTotal > 0 && (
              <div className="h-1.5 rounded-full overflow-hidden bg-theme-hover flex mt-1">
                ...
                {storagePct > 0 && <div className="h-full bg-teal-500" style={{ width: `${storagePct}%` }} />}
              </div>
            )}
